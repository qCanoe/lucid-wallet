...
完整工具规范请参考 `datasets/spec/tools.md`（符合 RFC 2119 标准）
...
项目包含符合 RFC 2119 约定的标准化数据集规范。完整文档请参考 `datasets/spec/`：

- **查询规范** (`spec/query.md`)：自然语言意图格式
- **元数据规范** (`spec/metadata.md`)：链、任务类型、难度、账户状态
- **约束规范** (`spec/constraints.md`)：用户约束 + 系统安全约束
- **工具与环境规范** (`spec/tools.md`)：Agent 可用的工具接口
- **输出格式规范** (`spec/output.md`)：交易序列输出格式

所有规范均使用规范性关键词（MUST, SHOULD, MAY），符合 RFC 2119 标准，确保清晰和精确。
